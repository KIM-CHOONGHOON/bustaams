...
CREATE TABLE `TB_AUCTION_REQ` (
  ...
  CONSTRAINT `FK_REQ_USER` FOREIGN KEY (`TRAVELER_ID`) REFERENCES `TB_USER` (`CUST_ID`)
...
CREATE TABLE `TB_AUCTION_REQ_BUS` (
  ...
  `RES_BUS_AMT` decimal(13,0) DEFAULT NULL COMMENT '차량별 견적요청금액',
...
CREATE TABLE `TB_BUS_DRIVER_VEHICLE` (
  ...
  `CUST_ID` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '소유 기사 (TB_USER)',
...
CREATE TABLE `TB_DRIVER_DETAIL` (
  `CUST_ID` varchar(10) NOT NULL COMMENT '기사 식별자',
...
CREATE TABLE `TB_DRIVER_DOCS` (
  `CUST_ID` varchar(10) NOT NULL COMMENT '기사 식별자 (ID)',
  ...
  `ORIGINAL_FILE_NM varchar(100) DEFAULT NULL,
  `ORIGINAL_FILE_TYPE` varchar(6) DEFAULT NULL,  
  `REG_DT` datetime DEFAULT CURRENT_TIMESTAMP  COMMENT '등록 일시',
  `REG_ID` varchar(10) DEFAULT NULL COMMENT '등록자 ID',
  `MOD_DT` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '수정 일시',
  `MOD_ID` varchar(10) DEFAULT NULL COMMENT '수정자 ID',
...
CREATE TABLE `TB_PAYMENT_CARD` (
  `CUST_ID` varchar(10) NOT NULL,
  `CARD_SEQ` int NOT NULL COMMENT '카드 순번',   
...
CREATE TABLE `TB_SMS_LOG` (
  ...
  `SEND_CATEGORY` enum('AUCTION','BIDDING','CONFIRM','DONE','TRAVELER_CANCEL','DRIVER_CANCEL','BUS_CHANGE','BUS_CANCEL','SIGN_UP','NEW_PASSWORD','OTHER') NOT NULL COMMENT 'AUCTION : 버스예약 요청등록,BIDDING : 버스예약 응찰등록,CONFIRM : 버스예약 확정,DONE    : 버스운행 정상종료,TRAVELER_CANCEL : 여행자 버스예약 전체취소,DRIVER_CANCEL : 버스기사 응찰취소,BUS_CHANGE : 여행자 버스 변경요청,BUS_CANCEL : 여행자 버스 취소(버스대수 감소),SIGN_UP : 신규가입,NEW_PASSWORD : 임시번호발송', OTHER  : 기타',
...
CREATE TABLE `TB_SUBSCRIPTION` (
  ...
  CONSTRAINT `FK_SUB_USER` FOREIGN KEY (`CUST_ID`) REFERENCES `TB_USER` (`CUST_ID`)
...
CREATE TABLE `TB_TRIP_REVIEW` (
  ...
  `REVIEW_SEQ` int NOT NULL AUTO_INCREMENT COMMENT '리뷰 SEQ', 
...
CREATE TABLE `TB_USER_CANCEL_HIST` (
  `CUST_ID` varchar(10) NOT NULL COMMENT '대상 고객 TB_USER.CUST_ID',
...
CREATE TABLE `TB_USER_CANCEL_MANAGE` (
  ...
  `REG_ID` varchar(10) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '등록자 ID',
  `MOD_DT` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '마스터 수정 일시',
  `MOD_ID` varchar(10) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '수정자 ID',
  PRIMARY KEY (`CUST_ID`),
