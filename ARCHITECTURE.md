...
- **STEP 2 (메타데이터 DB 제어):** 구글 내 파일 생성이 100% 성공하면 구글에서 반환되는 물리적인 객체 경로(URL)와 용량 등의 메타데이터를 백엔드 서버가 넘겨받아, 이를 먼저 전사 공통 파일 허브 테이블인 **`TB_FILE_MASTER`** 에 저장(INSERT) 하고 고유한 `SIGN_FILE_UUID` 값을 발급받습니다.
- **STEP 3 (비즈니스 결합 DB 제어):** 최종적으로 회원 가입 트랜잭션을 승인하고 회원을 등록시킬 때, 앞서 발급받은 그림 파일 식별자(UUID)를 회원의 약관 동의 상세 기록 테이블인 **`TB_USER_TERMS_HIST`** 에 외래키(FK)로 넣어 줍니다. 이로써 회원 데이터베이스와 구글 클라우드의 그림 파일 간의 법적 **결속(Binding)** 이 영구적으로 완성됩니다.
...
### 2. TB_USER 테이블 - 암호화 대상 컬럼

| 컬럼명 | 의미 | 암호화 방식 | 비고 |
|---|---|---|---|
| `USER_ID` | 이메일 (로그인 ID) | **AES-256-GCM** | 중복 검사 시 전체 복호화 비교 |
| `USER_NM` | 이름 | **AES-256-GCM** | 화면 표시 시 복호화 |
| `PHONE_NO` | 전화번호 | **AES-256-GCM` | 발신/표시 시 복호화 |
| `PASSWORD` | 비밀번호 | **bcrypt** | 단방향, 비교만 가능 |
...
- **주요 함수:**
  - `encrypt(plainText)` → DB 저장 시 호출 (회원가입 INSERT 직전)
  - `decrypt(encryptedText)` → 조회 후 화면 표시 시 호출
