...
const emailRegex = /const handleCheckEmail = async \(\) => \{[\s\S]*?setIsEmailChecked\(true\);[\s\S]*?\};/;
content = content.replace(emailRegex, `const handleCheckEmail = async () => {
...
const idRegex = /const handleCheckId = async \(\) => \{[\s\S]*?setIsIdChecked\(true\);[\s\S]*?\};/;
content = content.replace(idRegex, `const handleCheckId = async () => {
...
console.log('Fixed Signup.jsx syntax errors (v2)');
