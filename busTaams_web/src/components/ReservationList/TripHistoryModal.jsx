...
const TripHistoryModal = ({ user, onClose }) => {
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      ...
      'AUCTION': { label: '경매 진행중', class: 'bg-amber-100 text-amber-700' }
    };
  ...
  const filteredHistory = history.filter(item => {
    ...
    if (filter === 'COMPLETED') return item.REQ_STAT === 'COMPLETED' || item.REQ_STAT === 'CONFIRM';
    if (filter === 'CANCELED') return item.REQ_STAT === 'TRAVELER_CANCEL' || item.REQ_STAT === 'DRIVER_CANCEL';
    return true;
  });
          ...
          {loading ? (
            ...
            <div className="grid gap-6">
              {filteredHistory.map((item) => (
                ...
                >
                  {/* Status Bar */}
                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${item.REQ_STAT.includes('CANCEL') ? 'bg-rose-400' : 'bg-teal-600'}`}></div>
                  ...
                  <div className="flex flex-col items-center shrink-0">
                    ...
                    <span className="text-xs font-bold text-slate-400">{formatDate(item.REG_DT)}</span>
                  </div>

                  {/* Trip Info */}
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      {getStatusBadge(item.REQ_STAT)}
                  ...
                  <div className="flex flex-col items-end shrink-0 gap-4">
                    <div className="text-right">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">최종 이용 금액</p>
                      <p className="text-3xl font-headline font-black text-teal-900 tracking-tighter">
                        {formatPrice(item.TOTAL_AMT)}
                      </p>
                    </div>
