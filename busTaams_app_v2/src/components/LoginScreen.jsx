...
const LoginScreen = () => {
  ...
  const [userType, setUserType] = useState('TRAVELER'); // TRAVELER, DRIVER, PARTNER
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    try {
        const user = await login(formData.userId, formData.password, userType);
        localStorage.setItem('user', JSON.stringify(user));
        navigate('/dashboard');
    } catch (err) {
        setError(err.message || '인증 정보가 일치하지 않습니다.');
    } finally {
        setIsLoading(false);
    }
  };
    ...
    <div className="bg-background font-body text-on-background min-h-screen flex flex-col overflow-x-hidden">
      <header className="flex justify-between items-center w-full px-6 pt-8 pb-4 max-w-7xl mx-auto z-10">
        ...
        <div className="flex items-center gap-4">
          <span className="text-on-surface-variant font-medium text-sm">도움이 필요하신가요?</span>
          ...
          <section className="lg:col-span-7 hidden lg:block space-y-8">
            <div className="relative rounded-3xl overflow-hidden shadow-[0_40px_60px_-15px_rgba(0,104,95,0.12)]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent flex flex-col justify-end p-12">
                <h2 className="font-headline font-extrabold text-5xl text-white tracking-tight leading-tight mb-4">
                  이동의 미학, <br/>키네틱 갤러리.
                </h2>
                <p className="text-white text-lg max-w-md font-medium opacity-90">
          ...
          <section className="lg:col-span-5 w-full">
            <div className="space-y-12">
              <div className="space-y-4">
                <h1 className="font-headline font-extrabold text-4xl lg:text-5xl text-on-surface tracking-tighter">다시 오신 것을 환영합니다</h1>
                <p className="text-on-surface-variant text-lg">개인 대시보드에 접속하려면 자격 증명을 입력하세요.</p>
              </div>

              {/* 사용자 유형 선택 탭 추가 (기존 디자인 스타일 유지) */}
              <div className="flex bg-surface-container-high p-1 rounded-xl mb-6">
                {['TRAVELER', 'DRIVER', 'PARTNER'].map((type) => (
                  <button
                    key={type}
                  ...
                  >
                    {type === 'TRAVELER' ? '여행자' : type === 'DRIVER' ? '기사님' : '영업파트너'}
                  </button>
                ))}
              </div>

              {error && (
                <div className="bg-error/10 border border-error/20 text-error p-4 rounded-xl text-sm font-bold text-center animate-pulse">
                  {error}
                </div>
              )}
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      className="w-full bg-surface-container-high border-none rounded-xl py-4 pl-12 pr-4 focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all text-on-surface placeholder:text-outline/50 font-medium outline-none" 
                      id="user-id" 
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center ml-1">
                    ...
                    <button type="button" className="text-sm font-bold text-primary hover:text-primary-container transition-colors">키를 잊으셨나요?</button>
                  </div>
                  <div className="relative">
                ...
                <div className="pt-4">
                  <button 
                    disabled={isLoading}
                    className="w-full bg-primary text-white font-headline font-bold py-5 px-8 rounded-full shadow-[0_20px_40px_-10px_rgba(0,104,95,0.3)] hover:shadow-[0_25px_50px_-12px_rgba(0,104,95,0.4)] active:scale-[0.98] transition-all duration-300 text-lg flex justify-center items-center gap-2" 
                    type="submit"
                  >
                    {isLoading ? (
                ...
                <div className="flex items-center justify-center gap-4 mt-6 text-sm font-bold">
                  <button type="button" className="text-on-surface-variant hover:text-primary transition-colors">회원가입</button>
                  <div className="w-[1px] h-3 bg-outline/30"></div>
                  <button type="button" className="text-on-surface-variant hover:text-primary transition-colors">아이디 / 비밀번호 찾기</button>
                </div>
              </form>
