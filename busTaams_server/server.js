...
app.post('/api/auth/login', async (req, res) => {
    try {
        const { userId, password, snsType } = req.body;
        ...
        const [rows] = await pool.execute('SELECT * FROM TB_USER WHERE SNS_TYPE = ?', [snsType || 'NONE']);
        ...
        // USER_UUID를 HEX 문자열로 변환하여 반환
        const [uuidRows] = await pool.execute(
            'SELECT BIN_TO_UUID(USER_UUID) AS userUuid FROM TB_USER WHERE USER_ID_ENC = ?',
            [user.USER_ID_ENC]
        );
        const userUuid = uuidRows.length > 0 ? uuidRows[0].userUuid : null;
