...
const InquiryDetailCustomer = () => {
    ...
    if (!inquiry) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center font-body p-6 text-center">
                ...
                <button onClick={() => navigate(-1)} className="px-8 py-3 bg-teal-700 text-white rounded-xl btn-primary">돌아가기</button>
            </div>
        );
    }
    ...
    return (
        <div className="bg-background text-on-surface min-h-screen pb-40 font-body text-left">
            ...
            <main className="pt-24 px-6 max-w-3xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                    <div className="space-y-1">
                        <span className={`inline-flex items-center px-4 py-1.5 rounded-xl text-xs font-bold tracking-widest uppercase ${inquiry.isCompleted ? 'bg-teal-700 text-white shadow-lg shadow-teal-900/20' : 'bg-slate-200 text-slate-500'}`}>
                            {inquiry.statusText}
                        </span>
                ...
                <div className="space-y-8">
                    ...
                    <section className="bg-white rounded-2xl p-8 shadow-sm border border-slate-50 relative overflow-hidden">
                    ...
                    {inquiry.replyContent ? (
                        <section className="bg-slate-50 rounded-2xl p-8 relative border border-slate-100">
                            ...
                            <div className="pl-4">
                                ...
                                <div className="text-on-surface leading-relaxed font-medium bg-white/60 p-6 rounded-xl border border-white whitespace-pre-wrap">
                                    {inquiry.replyContent}
                                </div>
                                <div className="mt-8 flex justify-end">
                                    <button className="bg-gradient-to-br from-teal-700 to-teal-900 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-xl shadow-teal-900/20 active:scale-95 transition-all flex items-center gap-2 btn-primary">
                                        <span>도움이 되었습니다</span>
            ...
            <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[95%] md:w-[600px] rounded-2xl z-50 bg-white/80 backdrop-blur-xl shadow-2xl flex justify-around items-center p-2 h-16 border border-white/40">
                ...
                <button onClick={() => navigate('/inquiry-list')} className="flex flex-col items-center justify-center bg-teal-700 text-white rounded-xl px-5 py-2">
