...
const EstimateSubmissionDriver = () => {
                    ...
                    <aside className="lg:col-span-4 text-left">
                        <div className="bg-slate-900 rounded-[3.5rem] p-12 space-y-12 shadow-2xl shadow-slate-900/40 sticky top-48 text-left">
                            ...
                            <div className="space-y-10 text-left">
                                {[
                                    { icon: 'location_on', label: '출발지', value: '시애틀-타코마 국제공항 (SEA)' },
                                    { icon: 'route', label: '경유지', value: ['1: 수원역', '2: 대전복합터미널', '3: 동대구역'], isList: true },
                                    { icon: 'flag', label: '목적지', value: '밴쿠버 워터프런트, BC' },
                                    { icon: 'calendar_month', label: '출발 일시', value: '2024. 10. 24 • AM 09:00' },
                                        ...
                                        <div className="text-left">
                                            ...
                                            {item.isList ? (
                                                <div className="space-y-1 text-left">
                                                    {(item.value as string[]).map((v, idx) => (
                                                        <p key={idx} className="font-black text-white italic text-lg leading-none">{v}</p>
                                                    ))}
                                                </div>
                                            ) : (
                                                <p className="font-black text-white italic text-lg leading-tight text-left">{item.value as string}</p>
                                            )}
                                        </div>
                                    </div>
                                ))}
...
export default EstimateSubmissionDriver;
